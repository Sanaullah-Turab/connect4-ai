# ─────────────────────────────────────────────
#  ai.py  –  Minimax + Alpha-Beta + Heuristic
#  Completely separated from GUI and board UI
# ─────────────────────────────────────────────
...
def _score_window(window: list, piece: int) -> int:
    ...
    if opp_count == 3 and empty_count == 1:
        score += BLOCK_THREE          # negative, so this hurts our score
...
def evaluate_board(board, piece: int) -> int:
    ...
    # Center column preference — controlling the center is strategically strong
    center_col  = [g[r][COLS // 2] for r in range(ROWS)]
    score      += center_col.count(piece) * SCORE_CENTER
...
def minimax(board, depth: int, alpha: float, beta: float,
    ...
    if depth == 0:                         # leaf node – use heuristic
        return (None, evaluate_board(board, AI_PIECE))

    # ── Order moves: center columns first (improves pruning) ──
...
def get_ai_move(board) -> int:
    ...
    # Fallback safety: if minimax returns None (shouldn't happen), pick first valid
    if col is None:
